...
const MyProperties = () => {
    ...
    try {
      ...
      await Promise.all(
        transactions.map(async (transact) => {
          const propertyResponse = await axios.get(`http://localhost:3000/api/property/fetchproperty/${transact.property}`, {
            headers: {
              'auth-token': token,
            },
          });
  ...
  const sellerDetails = async (transactions) => {
    try {
      ...
      await Promise.all(
        transactions.map(async (transact) => {
          const sellerResponse = await axios.get(`http://localhost:3000/api/auth/getuser/${transact.prevowner}`, {
            headers: {
              'auth-token': token,
            },
          });
  ...
  const buyerDetails = async (transactions) => {
    try {
      ...
      await Promise.all(
        transactions.map(async (transact) => {
          const buyerResponse = await axios.get(`http://localhost:3000/api/auth/getuser/${transact.newowner}`, {
            headers: {
              'auth-token': token,
            },
          });
