...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (userIdFromToken) {
      axios
        .get(`http://localhost:3000/api/contract/seller/${userIdFromToken}`)
        .then((response) => {
  ...
  const verifyContract = (contractId) => {
    ...
    axios
      .post(`http://localhost:3000/api/contract/verify/${contractId}`)
      .then((response) => {
        // Handle the verification result, you can show a message to the user
        console.log('Contract verification result:', response.data);
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  const sellContract = (contractId) => {
    ...
    axios
      .post(`http://localhost:3000/api/contract/sell/${contractId}`)
      .then((response) => {
        // Handle the selling process result, you can show a message to the user
        console.log('Contract selling process result:', response.data);
      })
      .catch((error) => {
        console.error(error);
      });
